...
Создай ПОЛНЫЙ ОТЧЕТ ПО РЕЗУЛЬТАТАМ ПЕНТЕСТА в бизнес-формате на русском языке. Отчет должен содержать ТОЛЬКО указанную структуру, БЕЗ ПОВТОРОВ и лишних разделов.`;
  ...
  try {
    ...
    const fullReportIndex = cleanedReport.search(/##\s*ПОЛНЫЙ\s+ОТЧЕТ\s+ПО\s+РЕЗУЛЬТАТАМ\s+ПЕНТЕСТА/i);
    if (fullReportIndex !== -1) {
      cleanedReport = cleanedReport.substring(fullReportIndex);
    }
    
    // Удаляем все что после заключения (раздел 6)
    const conclusionIndex = cleanedReport.search(/###\s*6[\.\)]?\s*Заключение/i);
    if (conclusionIndex !== -1) {
      // Находим конец раздела "Заключение"
      const nextSectionMatch = cleanedReport.substring(conclusionIndex).match(/^###\s*6[\.\)]?\s*Заключение[\s\S]*?(?=\n##|\n---|$)/i);
      if (nextSectionMatch) {
