...
1. ОТЧЕТ ДОЛЖЕН БЫТЬ НА РУССКОМ ЯЗЫКЕ (использование английских названий уязвимостей, технических терминов и деталей разрешено)
2. НЕ ДУБЛИРУЙ разделы - каждый раздел должен быть представлен ТОЛЬКО ОДИН РАЗ
3. НЕ создавай дополнительные разделы вне указанной структуры
4. НЕ добавляй английские заголовки разделов типа "Authentication Analysis Report", "Security Assessment Report" и т.д.
5. НЕ повторяй информацию из одного раздела в другом
...
### 3. Детальный анализ найденных уязвимостей
   ...
   - **Название уязвимости** (четкое и понятное, можно на английском если это стандартное название типа XSS, SQL Injection)
   - **Критичность** (CRITICAL/HIGH/MEDIUM/LOW)
   - **Расположение** (URL, эндпоинт, компонент)
   - **Детальное описание** (что именно не так, почему это проблема) - НА РУССКОМ
   - **Техническое описание** (как воспроизвести, proof-of-concept) - НА РУССКОМ, команды/код можно на английском
   - **Бизнес-влияние** (какой ущерб может быть нанесен) - НА РУССКОМ
   - **Рекомендации по исправлению** (конкретные шаги для устранения) - НА РУССКОМ
...
- Добавлять разделы типа "Authentication Analysis Report", "Security Assessment Report"
