...
def fix_final_2():
    ...
    # Fix line 93-94: function signature
    if workflows_lines[92].strip() == 'export async function pentestPipelineWorkflow(':
        if workflows_lines[93].strip() == 'input: PipelineInput,':
            # Parameter is on separate line, need to fix
            workflows_lines[92] = 'export async function pentestPipelineWorkflow(input: PipelineInput'
            workflows_lines.pop(93)
            print("\n   ✅ Fixed function signature")
        elif 'input' not in workflows_lines[92]:
            # Add input parameter
            workflows_lines[92] = workflows_lines[92].replace('(', '(input: PipelineInput')
            print("\n   ✅ Added input parameter to function")
    
    # Check line 242
    print("\n   Line 242:")
    if 241 < len(workflows_lines):
        print(f"   {242}: {workflows_lines[241]}")
        # Show context
        print("\n   Context around line 242:")
        for i in range(238, min(245, len(workflows_lines))):
            print(f"   {i+1:3d}: {workflows_lines[i]}")
