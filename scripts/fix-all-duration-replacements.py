#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Fix all Duration.from() replacements properly"""
...
def fix_all_replacements():
    """Fix all Duration.from() replacements"""
    ssh = paramiko.SSHClient()
    ssh.set_missing_host_key_policy(paramiko.AutoAddPolicy())
    ssh.connect(SERVER_HOST, port=SERVER_PORT, username=SERVER_USER, password=SERVER_PASSWORD, timeout=10)
    
    print("=" * 80)
    print("FIXING ALL DURATION.FROM() REPLACEMENTS")
    print("=" * 80)
    ...
    # Show lines with Duration.from()
    workflows_lines = workflows.split('\n')
    print("\n   Lines with Duration.from():")
    for i, line in enumerate(workflows_lines):
        if 'Duration.from' in line:
            print(f"   Line {i+1}: {line}")
    
    # Replace all Duration.from() with strings
    workflows = re.sub(
    ...
    # Remove Duration import if exists
    workflows_lines = workflows.split('\n')
    ...
    with sftp.open('/opt/xaker/shannon/src/temporal/workflows.ts', 'w') as f:
        f.write(workflows.encode('utf-8'))
    
    # Verify replacements
    print("\n   After replacement:")
    workflows_lines = workflows.split('\n')
    for i, line in enumerate(workflows_lines):
        if 'initialInterval:' in line or 'maximumInterval:' in line:
