#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Fix retry policies using Duration helper from Temporal"""
...
def fix_with_helper():
    """Fix retry policies using Duration helper"""
    ssh = paramiko.SSHClient()
    ssh.set_missing_host_key_policy(paramiko.AutoAddPolicy())
    ssh.connect(SERVER_HOST, port=SERVER_PORT, username=SERVER_USER, password=SERVER_PASSWORD, timeout=10)
    
    print("=" * 80)
    print("FIXING RETRY POLICIES WITH DURATION HELPER")
    print("=" * 80)
    
    sftp = ssh.open_sftp()
    
    # Fix workflows.ts - use ms() helper or Duration.fromMilliseconds()
    ...
    # Check what's imported from @temporalio/workflow
    print("\n   Current imports:")
    for i, line in enumerate(workflows_lines[:30]):
        if 'import' in line and '@temporalio' in line:
            print(f"   Line {i+1}: {line}")
    
    # Temporal retry policies can use ms() helper or Duration
    # Let's use ms() helper which converts strings to Duration
    # First, check if ms is imported
    has_ms = False
    for i, line in enumerate(workflows_lines[:30]):
        if 'ms' in line and 'import' in line and '@temporalio' in line:
            has_ms = True
            print(f"   ✅ ms() already imported at line {i+1}")
            break
    
    if not has_ms:
        # Add ms import
        for i, line in enumerate(workflows_lines[:30]):
            if 'from "@temporalio/workflow"' in line:
                workflows_lines[i] = line.replace('}', ', ms }')
                print(f"   ✅ Added ms() import at line {i+1}")
                break
    
    # Replace strings with ms() calls
    for i, line in enumerate(workflows_lines):
        if 'initialInterval:' in line and "'" in line:
            # Replace '5 minutes' with ms('5 minutes')
            workflows_lines[i] = re.sub(
                r"initialInterval:\s*['\"](\d+\s+\w+)['\"]",
                r"initialInterval: ms('\1')",
                line
            )
        if 'maximumInterval:' in line and "'" in line:
            workflows_lines[i] = re.sub(
                r"maximumInterval:\s*['\"](\d+\s+\w+)['\"]",
                r"maximumInterval: ms('\1')",
                line
            )
